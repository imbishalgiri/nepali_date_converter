 ...
 * @ignore
 */
export function getYearFromIndex(yearIndex) {
 ...
 */
export function findPassedDays(year, month, date) {
    try {
        ...
        const pastMonthDays = yearDaysMapping[yearIndex + extraYear][COMPLETED_DAYS] -
            pastYearDays +
...
export function mapDaysToDate(daysPassed) {
    ...
    const yearIndex = yearDaysMapping.findIndex((year) => daysPassed > year[COMPLETED_DAYS] &&
        daysPassed <= year[COMPLETED_DAYS] + year[TOTAL_DAYS]);
    const monthRemainder = daysPassed - yearDaysMapping[yearIndex][COMPLETED_DAYS];
    const monthIndex = monthDaysMappings[yearIndex].findIndex((month) => monthRemainder > month[COMPLETED_DAYS] &&
...
export function findPassedDaysAD(year, month, date) {
    const timeDiff = Math.abs(Date.UTC(year, month, date) -
        Date.UTC(beginEnglish.year, beginEnglish.month, beginEnglish.date));
    const diffDays = Math.ceil(timeDiff / (1000 * 3600 * 24));
    return diffDays;
}
export function mapDaysToDateAD(daysPassed) {
...
export function convertToAD(bsDateObject) {
    try {
        ...
        return {
            AD,
            BS: Object.assign(Object.assign({}, BS), { day: AD.day }),
        };
    }
    catch (_a) {
        throw new Error("The date doesn't fall within 2000/01/01 - 2090/12/30");
...
export function convertToBS(adDateObject) {
    try {
...
export function format(bsDate, stringFormat, language) {
    return stringFormat
